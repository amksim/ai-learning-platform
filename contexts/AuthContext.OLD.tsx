...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Аварийный таймаут - если через 10 секунд всё ещё loading, принудительно выключаем
    const emergencyTimeout = setTimeout(() => {
      if (loading) {
        console.error('⚠️ АВАРИЙНЫЙ ТАЙМАУТ: checkUser не завершился за 10 секунд');
        setLoading(false);
  ...
  async function checkUser() {
    ...
    try {
      ...
      // КРИТИЧНО: finally ВСЕГДА выполнится, даже если был catch
      console.log('✅ checkUser завершен, setLoading(false)');
      setLoading(false);
      isCheckingUser.current = false;
    }
  }
